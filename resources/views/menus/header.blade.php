...
<ul @if ($stack == 0) class="nav navbar-nav navbar-right" @else class="dropdown-menu" @endif>
	...
	<li class="{{ count($menu_item->children) > 0 || isset($menu_item->app_model) ? 'dropdown' : '' }}">
		<a {{ count($menu_item->children) > 0 || isset($menu_item->app_model) ? 'class="dropdown-toggle"' : '' }} href='{{ !isset($menu_item->app_model) ? $menu_item->link() : 'javascript:void(0);' }}'>{{ $menu_item->title }} {!! count($menu_item->children) > 0 || isset($menu_item->app_model) ? '<b class="caret"></b>' : '' !!} {!! $menu_item->icon_class != null ? '<i class="'.$menu_item->icon_class.'"></i>':'' !!} </a>
		...
		<ul class="dropdown-menu">
			...
			<li><a href="{{route($menu_item->route,$item->slug)}}">{{$item->title}}</a></li>
			@endforeach
		</ul>
		@endif
		@endif
